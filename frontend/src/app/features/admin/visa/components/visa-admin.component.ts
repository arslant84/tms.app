...
import {
  ...
  type VisaFilters,
  type VisaProcessingDetails,
} from '../../../visa/services/visa.service';
...
export class VisaAdminComponent implements OnInit, OnDestroy {
  ...
  private loadSummaryData(): void {
    this.visaService
      ...
      .subscribe({
        next: response => {
          const apps: VisaApplication[] = Array.isArray(response) ? response : response.results;

          // Filter options
          const uniqueStatuses = [
            ...new Set<string>(apps.map(a => a.status).filter(Boolean)),
          ].sort();
          this.statusOptions = [
          ...
          const uniqueVisaTypes = [
            ...new Set<string>(apps.map(a => a.visa_type).filter(Boolean)),
          ].sort();
          this.visaTypeOptions = [
            { value: 'all', label: 'All Types' },
            ...uniqueVisaTypes.map(t => ({ value: t, label: t })),
          ];
          this.allDestinations = [
            ...new Set<string>(apps.map(a => a.destination).filter(Boolean)),
          ].sort();

          // Stats
          this.stats.total = apps.length;
          this.stats.pending = apps.filter(
            a =>
              a.status && (a.status.toLowerCase().includes('pending') || a.status === 'Submitted')
          ).length;
          this.stats.approved = apps.filter(a => a.status === 'Approved').length;
          this.stats.processing = apps.filter(
            a =>
              a.status &&
              (a.status.toLowerCase().includes('processing') || a.status === 'Under Review')
          ).length;
          this.stats.completed = apps.filter(a => a.status === 'Completed').length;
          this.stats.rejected = apps.filter(
  ...
  loadApplications(): void {
    ...
    this.visaService.getAllApplications(filters).subscribe({
      next: response => {
        ...
        this.totalApplications = Array.isArray(response)
    ...
    action$.subscribe({
      ...
      error: err => {
        this.toastService.error(
          this.errorHandler.getErrorMessage(err, 'Failed to process application')
        );
        this.processingId = null;
      },
    });
  ...
  private executeStartProcessing(application: VisaApplication): void {
    ...
    this.visaService
      ...
      .subscribe({
        ...
        error: err => {
          this.toastService.error(
            this.errorHandler.getErrorMessage(err, 'Failed to start processing')
          );
          this.processingId = null;
        },
      });
