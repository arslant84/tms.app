		...
		this.trfService.getAllTrfs({ adminView: true }).subscribe({
			next: (response: any) => {
				...
				const flightTrfs = trfs.filter((trf: any) => {
					...
					if (
						trf.travel_type === "Domestic" &&
						trf.domestic_travel_details?.itinerary?.length > 0
					) {
						return true;
					}
					return false;
				...
				this.stats = {
					...
					approved: flightTrfs.filter(
						(trf: any) => trf.status === "Approved" && !trf.has_flight_booking,
					).length,
					processing: flightTrfs.filter(
						(trf: any) => trf.has_flight_booking === true,
					).length,
				};
			},
			error: (err) => {
	...
	fetchFlightApplications(): void {
		...
		this.trfService.getAllTrfs({ adminView: true }).subscribe({
			next: (response: any) => {
				...
				const uniqueTypes = [
					...new Set(
						this.applications.map((a) => a.travelType).filter(Boolean),
					),
				].sort();
	...
	applyFilters(): void {
		this.filteredApplications = this.applications.filter((app) => {
			...
			const matchesType =
				...
				app.travelType === this.travelTypeFilter;
