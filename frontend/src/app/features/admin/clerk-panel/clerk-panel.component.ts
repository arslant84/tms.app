...
interface Task {
  ...
  // Fields vary by requestType (travel/accommodation/transport/visa) - the
  // detail panel template picks the ones relevant to whichever task is open.
  details: {
    ...
    checkInDate?: string;
    checkOutDate?: string;
    accommodationType?: 'ashgabat' | 'kiyanly' | string;
...
export class ClerkPanelComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.accommodationService.getAccommodationRequests().subscribe(requests => {
      ...
      const accommodationTasks = requests.map(request =>
        this.convertAccommodationRequestToTask(request)
      );
  ...
  startProcessing(): void {
    ...
    if (this.selectedTask.requestType === 'accommodation') {
      this.accommodationService
        .updateRequestStatus(this.selectedTask.id, 'in_progress')
        .subscribe(success => {
          if (success) {
  ...
  completeTask(): void {
    ...
    if (this.selectedTask.requestType === 'accommodation') {
      this.accommodationService
        .updateRequestStatus(this.selectedTask.id, 'completed')
        .subscribe(success => {
          if (success) {
            // Update local task data
            const index = this.tasks.findIndex(t => t.id === this.selectedTask!.id);
