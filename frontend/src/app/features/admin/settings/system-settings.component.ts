...
import { Component, type DoCheck, inject, type OnInit } from "@angular/core";
...
@Component({
	...
	imports: [
		...
		FormsModule,
		RouterModule,
		TmsApp_Admin_SystemSettings_RoleManagementComponent,
		DepartmentManagementComponent,
		EnhancedWorkflowConfigComponent,
				...
				if (Array.isArray(settingsData)) {
					...
					const formDataFromSettings = settingsData.reduce((acc, setting) => {
						if (Object.hasOwn(this.formData, setting.setting_key)) {
							...
							(acc as unknown as Record<string, string | boolean | number>)[
								setting.setting_key
							] = typedValue;
						}
						return acc;
					...
					this.toastService.error(
						"Failed to load application settings: Invalid data format",
					);
				}
		...
		this.settingsService.bulkUpdateSettings(updates).subscribe({
			next: () => {
				this.toastService.success(
					`${updates.length} setting${updates.length > 1 ? "s" : ""} updated successfully`,
				);
				this.loadSettings();
