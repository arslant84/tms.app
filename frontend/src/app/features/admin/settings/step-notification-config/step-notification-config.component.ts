...
export class StepNotificationConfigComponent implements OnInit {
  ...
  buildRecipientTypeOptions(): void {
    ...
    const roleOptions: { value: RecipientType; label: string; category: string }[] = this.availableRoles.map(role => ({
  ...
  getRecipientsByCategory(category: string): { value: RecipientType; label: string; category?: string }[] {
