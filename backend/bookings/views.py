...
from .models import BookingStatus, FlightBooking, HotelBooking
...
class HotelBookingViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if (
            user.is_superuser
            or can_view_all(user, "booking")
            or can_view_all(user, "accommodation")
        ):
            pass
        else:
            # Regular users can only see their own bookings
    ...
    def perform_create(self, serializer):
        """
        Booking is an admin/travel-desk function, not self-service - see
        FlightBookingViewSet.perform_create for the full rationale. Without
        this check, any authenticated user could create a HotelBooking
        against any other user's approved TRF.
        """
        user = self.request.user
        if not (
            user.is_superuser
            ...
            raise PermissionDenied(
                "Only booking/accommodation admins can create hotel bookings."
            )
        serializer.save()
    ...
    def confirm(self, request, pk=None):
        ...
        if (
            not request.user.is_superuser
            and not is_module_admin(request.user, "booking")
            and not is_module_admin(request.user, "accommodation")
        ):
            return forbidden_response(
                message="Only travel desk staff can confirm bookings"
            )
    ...
    def cancel(self, request, pk=None):
        ...
        is_admin = (
            request.user.is_superuser
            or is_module_admin(request.user, "booking")
            or is_module_admin(request.user, "accommodation")
        )
        if booking.user != request.user and not is_admin:
            return forbidden_response(message="You can only cancel your own bookings")
