...
class AdminReportsView(APIView):
    ...
    def get(self, request):
        ...
        accommodation_requests = AccommodationRequest.objects.filter(created_at__gte=start_date)
        ...
        previous_total = (
            TravelRequest.objects.filter(created_at__gte=previous_start, created_at__lt=start_date).count() +
            TransportRequest.objects.filter(created_at__gte=previous_start, created_at__lt=start_date).count() +
            VisaApplication.objects.filter(created_at__gte=previous_start, created_at__lt=start_date).count() +
            AccommodationRequest.objects.filter(created_at__gte=previous_start, created_at__lt=start_date).count()
        )

        # Calculate percentage change
        total_change = ((total_requests - previous_total) / previous_total * 100) if previous_total > 0 else 0
        ...
        processing_change = ((avg_processing_hours - previous_avg_hours) / previous_avg_hours * 100) if previous_avg_hours > 0 else 0

        # Calculate completion rate
        total_workflows = WorkflowInstance.objects.filter(started_at__gte=start_date).count()
        ...
        prev_completion_rate = (prev_completed / prev_total_workflows * 100) if prev_total_workflows > 0 else 0
        completion_change = completion_rate - prev_completion_rate

        # Pending requests
        pending_count = (
            trf_requests.exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count() +
            transport_requests.exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count() +
            visa_requests.exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count() +
            accommodation_requests.exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count()
        )
        ...
        pending_change = ((pending_count - previous_pending) / previous_pending * 100) if previous_pending > 0 else 0
        ...
        report_data = {
            'key_metrics': [
                ...
                },
                {
                    ...
                    'trend': 'down' if processing_change < 0 else 'up' if processing_change > 0 else 'neutral',
                    'icon': 'bi-clock'
                },
                {
                    ...
                    'trend': 'up' if completion_change > 0 else 'down' if completion_change < 0 else 'neutral',
                ...
                },
                {
                    ...
                    'trend': 'down' if pending_change < 0 else 'up' if pending_change > 0 else 'neutral',
    ...
    def _get_monthly_trends(self):
        ...
        for i in range(11, -1, -1):
            month_start = timezone.datetime(now.year, now.month, 1, tzinfo=timezone.get_current_timezone()) - timedelta(days=30 * i)
            ...
            submitted = (
                TravelRequest.objects.filter(created_at__gte=month_start, created_at__lt=month_end).count() +
                TransportRequest.objects.filter(created_at__gte=month_start, created_at__lt=month_end).count() +
                VisaApplication.objects.filter(created_at__gte=month_start, created_at__lt=month_end).count() +
                AccommodationRequest.objects.filter(created_at__gte=month_start, created_at__lt=month_end).count()
            )
    ...
    def _get_top_performers(self, start_date):
        ...
        approvers = WorkflowStepExecution.objects.filter(
            status='approved',
            action_date__gte=start_date,
            actioned_by__isnull=False
        ).values('actioned_by').annotate(
            processed=Count('id')
...
class DepartmentalReportsView(APIView):
    ...
    def get(self, request):
        ...
        for dept_id in department_ids:
            ...
            total_requests = trf_count + transport_count + visa_count + accommodation_count

            # Get status breakdown
            approved = (
                TravelRequest.objects.filter(created_by__in=dept_users, created_at__gte=start_date, status='Approved').count() +
                TransportRequest.objects.filter(requestor__in=dept_users, created_at__gte=start_date, status='Approved').count() +
                VisaApplication.objects.filter(user__in=dept_users, created_at__gte=start_date, status='Approved').count() +
                AccommodationRequest.objects.filter(trf__created_by__in=dept_users, created_at__gte=start_date, status='Approved').count()
            )

            pending = (
                TravelRequest.objects.filter(created_by__in=dept_users, created_at__gte=start_date).exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count() +
                TransportRequest.objects.filter(requestor__in=dept_users, created_at__gte=start_date).exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count() +
                VisaApplication.objects.filter(user__in=dept_users, created_at__gte=start_date).exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count() +
                AccommodationRequest.objects.filter(trf__created_by__in=dept_users, created_at__gte=start_date).exclude(status__in=['Approved', 'Rejected', 'Cancelled']).count()
            )

            rejected = (
                TravelRequest.objects.filter(created_by__in=dept_users, created_at__gte=start_date, status='Rejected').count() +
                TransportRequest.objects.filter(requestor__in=dept_users, created_at__gte=start_date, status='Rejected').count() +
                VisaApplication.objects.filter(user__in=dept_users, created_at__gte=start_date, status='Rejected').count() +
                AccommodationRequest.objects.filter(trf__created_by__in=dept_users, created_at__gte=start_date, status='Rejected').count()
            )
            ...
            for user in dept_users[:10]:  # Top 10 users
                user_requests = (
                    TravelRequest.objects.filter(created_by=user, created_at__gte=start_date).count() +
                    TransportRequest.objects.filter(requestor=user, created_at__gte=start_date).count() +
                    VisaApplication.objects.filter(user=user, created_at__gte=start_date).count()
                )
                if user_requests > 0:
...
class UserActivityReportsView(APIView):
    ...
    def get(self, request):
        ...
        for user in users:
            ...
            # Last activity
            last_request = TravelRequest.objects.filter(created_by=user).order_by('-created_at').first()
            last_approval = WorkflowStepExecution.objects.filter(actioned_by=user).order_by('-action_date').first()

            last_activity_date = None
            if last_request and last_approval:
                last_activity_date = max(last_request.created_at, last_approval.action_date or last_approval.created_at)
            elif last_request:
                last_activity_date = last_request.created_at
            elif last_approval:
                last_activity_date = last_approval.action_date or last_approval.created_at

            activity['lastActivityDate'] = last_activity_date.isoformat() if last_activity_date else None

            user_activities.append(activity)

        # Sort by total activity (requests + approvals)
        user_activities.sort(
            key=lambda x: x.get('requestsSubmitted', {}).get('total', 0) + x.get('approvalsProcessed', {}).get('total', 0),
            reverse=True
        )
...
class FinancialSummaryReportsView(APIView):
    ...
    def get(self, request):
        ...
        flight_cost = flight_query.aggregate(
            total=Sum('cost')
        )['total'] or 0
        ...
        hotel_cost = hotel_query.aggregate(
            total=Sum('cost_per_night')
        )['total'] or 0
        ...
        estimated_transport_cost = transport_count * 50  # $50 per transport request estimate
        ...
        accommodation_query = AccommodationRequest.objects.filter(created_at__gte=start_date)
        if dept_users:
            accommodation_query = accommodation_query.filter(trf__created_by__in=dept_users)

        accommodation_count = accommodation_query.count()
        ...
        for dept_id in department_ids:
            ...
            dept_hotels = HotelBooking.objects.filter(
                trf__in=dept_trfs,
                created_at__gte=start_date
            ).aggregate(total=Sum('cost_per_night'))['total'] or 0

            dept_transport = TransportRequest.objects.filter(
                requestor__in=dept_users,
                created_at__gte=start_date
            ).count() * 50

            dept_visa = VisaApplication.objects.filter(
                user__in=dept_users,
                created_at__gte=start_date
            ).count() * 200

            dept_accommodation = AccommodationRequest.objects.filter(
                trf__created_by__in=dept_users,
                created_at__gte=start_date
            ).count() * 300  # Simplified estimate

            dept_total = dept_flights + dept_hotels + dept_transport + dept_visa + dept_accommodation
        ...
        for i in range(11, -1, -1):
            ...
            month_flights = FlightBooking.objects.filter(
                created_at__gte=month_start,
                created_at__lt=month_end
            ).aggregate(total=Sum('cost'))['total'] or 0

            month_hotels = HotelBooking.objects.filter(
                created_at__gte=month_start,
                created_at__lt=month_end
            ).aggregate(total=Sum('cost_per_night'))['total'] or 0
...
class ReportExportView(APIView):
    ...
    def _export_csv(self, report_type, data):
        ...
        if report_type == 'departmental':
            writer.writerow(['Department', 'Total Requests', 'Travel', 'Transport', 'Visa', 'Accommodation',
                           'Approved', 'Pending', 'Rejected', 'Avg Processing Time (hrs)', 'Active Users'])
            ...
            writer.writerow(['Name', 'Email', 'Department', 'Requests Submitted', 'Approvals Processed', 'Last Activity'])
    ...
    def _export_excel(self, report_type, data):
        ...
        # Add headers and rows based on report type
        if report_type == 'departmental':
            headers = ['Department', 'Total Requests', 'Travel', 'Transport', 'Visa', 'Accommodation',
                      'Approved', 'Pending', 'Rejected', 'Avg Processing Time (hrs)', 'Active Users']
            ws.append(headers)
            ...
            headers = ['Name', 'Email', 'Department', 'Requests Submitted', 'Approvals Processed', 'Last Activity']
            ws.append(headers)
    ...
    def _export_pdf(self, report_type, data):
        ...
        if report_type == 'departmental':
            headers = ['Department', 'Total', 'Travel', 'Transport', 'Visa', 'Accom.',
                      'Approved', 'Pending', 'Rejected', 'Avg Time (hrs)', 'Users']
            table_data = [headers]
            ...
            headers = ['Name', 'Email', 'Department', 'Requests', 'Approvals', 'Last Activity']
            table_data = [headers]
            ...
            for user in data.get('users', []):
                table_data.append([
                    ...
                    user.get('lastActivityDate', '')[:10] if user.get('lastActivityDate') else ''
                ])
            ...
            table_data.append(['Accommodation', f"${breakdown.get('accommodation', 0):,.2f}"])
            ...
            if data.get('departmentBreakdown'):
                ...
                for dept in data.get('departmentBreakdown', []):
                    table_data.append([dept['department'], f"${dept['totalCost']:,.2f}"])
            ...
            for metric in data.get('key_metrics', []):
                trend_symbol = '↑' if metric['trend'] == 'up' else '↓' if metric['trend'] == 'down' else '→'
                table_data.append([
                    metric['name'],
                    str(metric['value']),
                    f"{metric['change']}%",
            ...
            if data.get('department_stats'):
                ...
                for dept in data.get('department_stats', [])[:5]:  # Top 5
                    completion_rate = round((dept['completed'] / dept['total'] * 100)) if dept['total'] > 0 else 0
                    table_data.append([
                        dept['department'],
                        str(dept['total']),
                        f"{completion_rate}%",
        ...
        elements.append(Paragraph(f"Generated on {timestamp} | Travel Management System", footer_style))
