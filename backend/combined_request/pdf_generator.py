...
def generate_combined_request_pdf(cr) -> HttpResponse:
        ...
        Table(
            [
                ...
                [
                    "Email",
                    cr.email
                    or (cr.requestor.email if cr.requestor else "Not provided"),
                ],
        ...
        Table(
            [
                ...
                [
                    "Submitted",
                    (
                        cr.submitted_at.strftime("%Y-%m-%d %H:%M")
                        if cr.submitted_at
                        else "Not submitted"
                    ),
                ],
        ...
        elements.append(
            Table(
                [
                    ...
                    [
                        "Departure Date",
                        str(cr.departure_date) if cr.departure_date else "-",
                    ],
    ...
    if cr.include_transport:
        ...
        if segments.exists():
            ...
            seg_data = [["#", "Date", "From", "To", "Time", "Pax"]]
            for seg in segments:
                seg_data.append(
                    [
                        str(seg.segment_order),
            ...
            Table(
                [
                    ...
                    [
                        "Check-in",
                        (
                            str(cr.accommodation_checkin)
                            if cr.accommodation_checkin
                            else "-"
                        ),
                    ],
                    [
                        "Check-out",
                        (
                            str(cr.accommodation_checkout)
                            if cr.accommodation_checkout
                            else "-"
                        ),
                    ],
                    [
                        "Special Requests",
                        (cr.accommodation_special_requests or "-")[:80],
                    ],
                ],
                colWidths=two_col,
                style=table_style,
            ...
            Table(
                [
                    ...
                    [
                        "Passport Expiry",
                        (
                            str(cr.visa_passport_expiry_date)
                            if cr.visa_passport_expiry_date
                            else "-"
                        ),
                    ],
            ...
            for step in workflow_instance.step_executions.all().order_by(
                ...
                approval_data.append(
                    [
                        ...
                        (
                            step.action_date.strftime("%Y-%m-%d")
                            if step.action_date
                            else "-"
                        ),
                        (step.comments or "-")[:30],
