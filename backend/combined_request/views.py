...
from utils.request_id_generator import generate_request_id, extract_context_for_combined
from utils.api_response import (
    success_response, error_response, validation_error_response,
    created_response, not_found_response, server_error_response
)
...
class CombinedRequestViewSet(viewsets.ModelViewSet):
    ...
    search_fields = ['requestor_name', 'department', 'destination_city', 'request_number', 'staff_id']
    ...
    def get_object(self):
        ...
        try:
            ...
            try:
                ...
                raise NotFound(f'Combined request {request_identifier} not found or you do not have permission to access it')
            except CombinedRequest.DoesNotExist:
                raise NotFound(f'Combined request not found with identifier: {lookup_value}')
    ...
    def get_queryset(self):
        ...
        if self.action in ['approve', 'reject']:
            logger.info(f"Approval action: Allowing access to all requests")
            return queryset

        # For retrieve, check view_all permission or pending approvals
        if self.action == 'retrieve':
            if can_view_all(user, 'combined'):
                logger.info(f"Retrieve: User has view_all_combined - allowing full access")
                return queryset
            pending_approval_ids = WorkflowApprovalHelper.get_pending_entity_ids_for_user(user, CombinedRequest)
        ...
        if admin_view and user.role:
            ...
            queryset = queryset.filter(requestor=user)
            logger.info(f"Personal view: Showing only user's own requests")
    ...
    def update(self, request, *args, **kwargs):
        ...
        is_editable = (
            current_status in ('Draft', 'Rejected') or
    ...
    def perform_create(self, serializer):
        ...
        if not validated_data.get('department'):
            validated_data['department'] = user.department.name if user.department else ''
        ...
        if status_value not in ['Draft']:
            ...
            try:
                ...
                if workflow_instance:
                    ...
                    logger.info(f"Workflow started for Combined Request #{combined_request.id}")
    ...
    def submit(self, request, pk=None):
        ...
        if not combined_request.request_number:
            try:
                ...
                logger.info(f"Generated request number: {combined_request.request_number}")
            except Exception as e:
    ...
    def approve(self, request, pk=None):
        ...
        try:
            ...
            logger.debug(f"User: {request.user.email}, Workflow found: {workflow_instance is not None}")
            ...
            if workflow_instance:
                ...
                if current_step:
                    ...
                    approval_step, created = CombinedRequestApprovalStep.objects.get_or_create(
                        combined_request=combined_request,
                ...
                # Legacy fallback
                logger.warning(f"No workflow instance found, using legacy approval")
    ...
    def process_module(self, request, pk=None):
        ...
        if not (request.user.is_admin or request.user.is_staff):
            user_perms = set(
                request.user.role.permissions.values_list('name', flat=True)
            ) if getattr(request.user, 'role', None) else set()
    ...
    def upload_document(self, request, pk=None):
        ...
        return success_response(data=serializer.data, message='Document uploaded successfully',
    ...
    def delete_document(self, request, pk=None, doc_id=None):
        ...
        combined_request = self.get_object()
        try:
            doc = CombinedRequestDocument.objects.get(id=doc_id, combined_request=combined_request)
    ...
    def approval_history(self, request, pk=None):
        ...
        workflow_instances = WorkflowInstance.objects.filter(
            content_type=content_type,
            object_id=combined_request.id
        ).prefetch_related('step_executions__workflow_step', 'step_executions__actioned_by')

        history = []
        for workflow in workflow_instances:
            for step_exec in workflow.step_executions.all().order_by('workflow_step__step_order'):
                history.append({
                    'step_order': step_exec.workflow_step.step_order,
                    'step_name': step_exec.workflow_step.step_name,
                    'status': step_exec.status,
                    'assigned_to': step_exec.assigned_to.name if step_exec.assigned_to else None,
                    'actioned_by': step_exec.actioned_by.name if step_exec.actioned_by else None,
                    'action_date': step_exec.action_date.isoformat() if step_exec.action_date else None,
    ...
    def pending_approvals(self, request):
        ...
        approval_statuses = [
            'Pending', 'Pending Line Manager', 'Pending HOD',
            'Pending Transport Review', 'Pending Accommodation Review',
            'Pending Visa Review', 'Submitted', 'Under Review'
        ]
        ...
        if user.is_superuser or is_module_admin(user, 'combined'):
            ...
            pending_ids = WorkflowApprovalHelper.get_pending_entity_ids_for_user(user, CombinedRequest)
            queryset = CombinedRequest.objects.filter(id__in=pending_ids).order_by('-submitted_at')
...
class CombinedRequestPassportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if combined_request_id:
            return CombinedRequestPassport.objects.filter(combined_request_id=combined_request_id)
...
class CombinedRequestItineraryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        combined_request_id = self.request.query_params.get('combined_request', None)
        if combined_request_id:
            return CombinedRequestItinerary.objects.filter(combined_request_id=combined_request_id)
...
class CombinedRequestTransportSegmentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        combined_request_id = self.request.query_params.get('combined_request', None)
        if combined_request_id:
            return CombinedRequestTransportSegment.objects.filter(combined_request_id=combined_request_id)
...
class CombinedRequestDocumentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        combined_request_id = self.request.query_params.get('combined_request', None)
        if combined_request_id:
            return CombinedRequestDocument.objects.filter(combined_request_id=combined_request_id)
