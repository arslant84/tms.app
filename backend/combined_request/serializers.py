...
class CombinedRequestPassportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'full_name', 'passport_number', 'nationality',
            'date_of_birth', 'place_of_birth', 'issue_date', 'expiry_date',
            'passport_file', 'created_at', 'updated_at'
        ]
...
class CombinedRequestItinerarySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'segment_order', 'segment_date', 'day_of_week',
            'from_location', 'to_location', 'departure_time', 'arrival_time',
            'mode_of_travel', 'flight_number', 'flight_class',
...
class CombinedRequestTransportSegmentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'segment_order', 'day_of_week', 'transport_type',
            'pickup_location', 'dropoff_location',
            'pickup_date', 'pickup_time', 'passengers', 'vehicle_type_preference',
            'notes', 'vehicle_number', 'driver_name', 'driver_contact',
            'segment_cost', 'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at']
...
class CombinedRequestDocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'document_type', 'module', 'file', 'file_name',
...
class CombinedRequestApprovalStepSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'step_order', 'step_name', 'step_role', 'module',
            'approver', 'approver_name', 'status', 'comments',
...
class CombinedRequestListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'department', 'status', 'included_modules',
            'travel_purpose', 'transport_purpose',
            'departure_date', 'return_date', 'destination_city',
...
class CombinedRequestDetailSerializer(serializers.ModelSerializer):
    ...
    transport_segments = CombinedRequestTransportSegmentSerializer(many=True, read_only=True)
...
class CombinedRequestCreateSerializer(serializers.ModelSerializer):
    ...
    transport_segments = CombinedRequestTransportSegmentSerializer(many=True, required=False)

    class Meta:
        model = CombinedRequest
        exclude = ['request_number', 'requestor', 'status', 'submitted_at', 'created_at', 'updated_at']
    ...
    def update(self, instance, validated_data):
        ...
        if itinerary_data is not None:
            ...
            for seg in itinerary_data:
                CombinedRequestItinerary.objects.create(combined_request=instance, **seg)

        if transport_data is not None:
            instance.transport_segments.all().delete()
            for seg in transport_data:
                CombinedRequestTransportSegment.objects.create(combined_request=instance, **seg)
