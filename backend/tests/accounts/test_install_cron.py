...
class TestInstallCron:
    def test_install_creates_three_jobs_and_writes(self, mock_crontab):
        mock_cron, mock_job = mock_crontab

        call_command("install_cron")

        assert mock_cron.new.call_count == 3
        assert mock_job.setall.call_count == 3
    ...
    def test_install_removes_existing_tagged_jobs_first(self, mock_crontab):
        ...
        assert mock_cron.remove_all.call_count == 3
        tags = {
    ...
    def test_schedules_match_crontab_example(self, mock_crontab):
        ...
        assert installed_schedules == ["0 2 * * *", "0 3 * * 0", "0 4 1 * *"]
