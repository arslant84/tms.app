...
class TestTransportRequestUpdate:
    ...
    def test_update_transport_request(self, authenticated_client, regular_user):
        ...
        create_response = authenticated_client.post('/api/transport/', {
            'transport_type': 'Airport Transfer',
            'pickup_location': 'Office',
            'dropoff_location': 'Airport',
            'pickup_date': '2026-03-01',
        ...
        if create_response.status_code in [status.HTTP_201_CREATED, status.HTTP_200_OK]:
            transport_id = create_response.json().get('data', {}).get('id') or create_response.json().get('id')
            if transport_id:
                response = authenticated_client.patch(f'/api/transport/{transport_id}/', {
