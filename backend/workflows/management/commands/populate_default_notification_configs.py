...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        templates = {}
        for template_name in set(cfg['template_name'] for cfg in self.DEFAULT_CONFIGS.values()):
            try:
                templates[template_name] = NotificationTemplate.objects.get(name=template_name)
            except NotificationTemplate.DoesNotExist:
                self.stdout.write(
                    self.style.WARNING(f"Template '{template_name}' not found - will skip configs using it")
                )
        ...
        for step in steps:
            ...
            for event_type, config in self.DEFAULT_CONFIGS.items():
                ...
                if not template:
                    self.stdout.write(
                        self.style.WARNING(f"  [{event_type}] Template not found - skipping")
                    )
                    skipped_count += 1
                    continue
                ...
                if dry_run:
                    self.stdout.write(
                        self.style.SUCCESS(f"  [{event_type}] Would create config with template '{config['template_name']}'")
                    )
                else:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"  [{event_type}] Created config with template '{config['template_name']}'")
                    )
        ...
        self.stdout.write("\n" + "=" * 60)
        if dry_run:
            self.stdout.write(self.style.WARNING(f"DRY RUN: Would create {created_count} configs"))
        else:
            self.stdout.write(self.style.SUCCESS(f"Created {created_count} notification configs"))
        self.stdout.write(f"Skipped {skipped_count} (already exist or template missing)")
