"""
Shared notification-dispatch machinery: resolving
WorkflowStepNotificationConfig, rendering templates, and falling back to
default per-event notification behavior when no config exists. Used by the
notify_* triggers in workflows/notifications.py.

Split out of workflows/notifications.py (see
docs/CODEBASE_REFACTOR_ROADMAP.md item 2) - a pure move, no logic changed.
`trigger_configured_notifications` is also re-exposed as a staticmethod on
`WorkflowNotifications` in notifications.py so existing external callers
(workflows/engine.py) don't need to change.
"""
...
def trigger_configured_notifications(
    ...
    try:
        ...
        if not configs.exists():
            ...
            _send_default_notification(step_execution, event_type)
            return
        ...
        for config in configs:
            try:
                ...
                context = _build_notification_context(step_execution)
                if context_overrides:
                    context.update(context_overrides)

                # Render template
                title = _render_template(template.subject, context)
                message = _render_template(template.body, context)
