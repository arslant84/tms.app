...
class WorkflowConditionInline(admin.TabularInline):
    ...
    extra = 0
    fields = ['condition_type', 'field_name', 'field_value', 'logical_operator', 'is_active']
...
class WorkflowTemplateAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'name', 'entity_type', 'is_active', 'allow_parallel_steps',
        'step_count', 'created_by', 'created_at'
    ]
...
class WorkflowStepAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('SLA Configuration', {
            'fields': ('sla_hours', 'escalation_hours')
        }),
        ('Timestamps', {
...
class WorkflowConditionAdmin(admin.ModelAdmin):
    ...
    list_display = [
        ...
        'field_value', 'logical_operator', 'is_active'
    ]
...
class WorkflowInstanceAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'workflow_template', 'status', 'current_step_order',
        'entity_display', 'initiated_by', 'started_at'
    ]
...
class WorkflowStepExecutionAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'workflow_instance', 'workflow_step_name', 'status',
        'assigned_to', 'actioned_by', 'action_date', 'is_overdue'
    ]
    list_filter = ['status', 'is_overdue', 'is_escalated', 'action_date']
    search_fields = [
        'workflow_instance__id', 'workflow_step__step_name',
        'assigned_to__email', 'actioned_by__email'
    ]
    ...
    fieldsets = (
        ('Workflow Instance', {
            'fields': ('workflow_instance', 'workflow_step')
        }),
        ('Status', {
            'fields': ('status',)
        }),
        ('Assignment', {
            'fields': ('assigned_to', 'actioned_by', 'action_date')
        }),
        ('Comments', {
            'fields': ('comments',)
        }),
        ('SLA Tracking', {
            'fields': ('sla_due_date', 'escalation_date', 'is_overdue', 'is_escalated')
        }),
        ('Timestamps', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class WorkflowDelegationAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'workflow_step_execution', 'delegated_from', 'delegated_to',
        'is_active', 'delegated_at', 'expires_at'
    ]
    list_filter = ['is_active', 'delegated_at', 'expires_at']
    search_fields = [
        'delegated_from__email', 'delegated_to__email', 'reason'
    ]
...
class WorkflowAuditLogAdmin(admin.ModelAdmin):
    ...
    list_display = [
        'id', 'workflow_instance', 'action_type', 'performed_by',
        'created_at'
    ]
    list_filter = ['action_type', 'created_at']
    search_fields = [
        'workflow_instance__id', 'action_description',
        'performed_by__email'
    ]
    readonly_fields = ['created_at']
    ordering = ['-created_at']
    ...
    fieldsets = (
        ...
        ('Action', {
            'fields': ('action_type', 'action_description', 'performed_by')
        }),
        ('State Snapshot', {
            'fields': ('workflow_state_snapshot',),
            'classes': ('collapse',)
        }),
        ('Context', {
