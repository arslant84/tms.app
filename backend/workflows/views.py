...
from .serializers import (
    WorkflowTemplateSerializer, WorkflowTemplateDetailSerializer,
    WorkflowTemplateCreateSerializer, WorkflowStepSerializer,
    WorkflowConditionSerializer, WorkflowInstanceSerializer,
    WorkflowInstanceDetailSerializer, WorkflowInstanceCreateSerializer,
    WorkflowStepExecutionSerializer, WorkflowDelegationSerializer,
    WorkflowAuditLogSerializer, WorkflowActionSerializer
)
...
class WorkflowInstanceViewSet(viewsets.ModelViewSet):
    ...
    def cancel(self, request, pk=None):
        ...
        instance.step_executions.filter(status='pending').update(
            status='cancelled'
        )
...
class WorkflowStepExecutionViewSet(viewsets.ModelViewSet):
    ...
    def take_action(self, request, pk=None):
        ...
        if action == 'skip':
            ...
            step_execution.actioned_by = user
            ...
            WorkflowAuditLog.objects.create(
                ...
                action_description=f'Step {workflow_step.step_name} skipped',
                performed_by=user
            )
        ...
        # Handle approve
        if action == 'approve':
            step_execution.status = 'approved'
            step_execution.actioned_by = user
            step_execution.action_date = timezone.now()
            step_execution.comments = comments
            step_execution.save()

            # Check if all steps for current order are complete
            current_order_steps = instance.step_executions.filter(
                workflow_step__step_order=instance.current_step_order
            )

            if not current_order_steps.filter(status='pending').exists():
                # All current steps complete, advance workflow
                self._advance_workflow(instance)
            ...
            WorkflowAuditLog.objects.create(
                ...
                action_description=f'Step {workflow_step.step_name} approved',
                performed_by=user
            )
        ...
        # Handle reject
        if action == 'reject':
            step_execution.status = 'rejected'
            step_execution.actioned_by = user
            step_execution.action_date = timezone.now()
            step_execution.comments = comments
            step_execution.save()
            ...
            instance.step_executions.filter(status='pending').exclude(
            ...
            WorkflowAuditLog.objects.create(
                ...
                action_description=f'Workflow rejected at step {workflow_step.step_name}',
                performed_by=user
            )
    ...
    def _advance_workflow(self, instance):
        """
        Advance workflow to next step or complete
        """
        max_step_order = instance.workflow_template.steps.aggregate(
            max_order=Max('step_order')
        )['max_order']

        if instance.current_step_order >= max_step_order:
            # Workflow complete
            ...
            WorkflowAuditLog.objects.create(
                workflow_instance=instance,
                action_type='approved',
                action_description='Workflow completed successfully',
                performed_by=None
            )
        else:
            # Move to next step
            ...
            # Update next step executions to pending if assigned
            next_steps = instance.step_executions.filter(
                workflow_step__step_order=instance.current_step_order
            )

            for step_exec in next_steps:
                if step_exec.assigned_to or step_exec.workflow_step.approver_role:
                    step_exec.status = 'pending'

                    # Set SLA dates
                    if step_exec.workflow_step.sla_hours:
                        step_exec.sla_due_date = timezone.now() + timedelta(
                            hours=step_exec.workflow_step.sla_hours
                        )

                    if step_exec.workflow_step.escalation_hours:
                        step_exec.escalation_date = timezone.now() + timedelta(
                            hours=step_exec.workflow_step.escalation_hours
                        )
