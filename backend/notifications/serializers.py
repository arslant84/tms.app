...
class NotificationTemplateSerializer(serializers.ModelSerializer):
    ...
    event_type_detail = NotificationEventTypeSerializer(
        source="event_type", read_only=True
    )
    event_type_name = serializers.CharField(source="event_type.name", read_only=True)
    event_type_module = serializers.CharField(
        source="event_type.module", read_only=True
    )
    ...
    def validate_name(self, value):
        ...
        if instance:
            ...
            if (
                NotificationTemplate.objects.exclude(pk=instance.pk)
                .filter(name=value)
                .exists()
            ):
...
class UserNotificationCreateSerializer(serializers.Serializer):
    ...
    def validate_event_type_id(self, value):
        ...
        if value:
            if not NotificationEventType.objects.filter(
