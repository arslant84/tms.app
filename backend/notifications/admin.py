...
class UserNotificationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("User", {"fields": ("user", "event_type")}),
        ("Content", {"fields": ("title", "message", "priority")}),
        ...
        (
            "Additional",
            {"fields": ("additional_data", "expires_at"), "classes": ("collapse",)},
        ),
    ...
    def mark_as_read(self, request, queryset):
        """Mark selected notifications as read - deletes them, matching the
        end-user mark-as-read behavior (notifications/views.py): a read
        notification has nothing left to act on, so it shouldn't linger
        with is_read=True instead of being removed."""
        count = queryset.count()
        queryset.delete()
        self.message_user(
            request, f"{count} notification(s) marked as read.", messages.SUCCESS
        )
    ...
    def delete_selected_notifications(self, request, queryset):
        ...
        queryset.delete()
        self.message_user(
            request, f"{count} notification(s) deleted.", messages.SUCCESS
        )
    ...
    def create_test_notifications(self, request, queryset):
        ...
        if not users.exists():
            self.message_user(
                request, "No users found in selected notifications.", messages.WARNING
            )
            return
        ...
        transport_id = (
            TransportRequest.objects.values_list("id", flat=True).first() or "demo"
        )
        ...
        templates = [
            {
                ...
                "message": "This is a test travel request notification.",
                "priority": "normal",
                "url": f"/trf/{trf_id}",
            },
            {
            ...
            },
            {
                ...
                "message": "This is a test approval confirmation.",
                "priority": "normal",
                "url": f"/expenses/{expense_id}",
            },
            {
                "title": "🚨 Test: Urgent Alert",
                "message": "This is a test urgent notification.",
                "priority": "urgent",
                "url": f"/transport/{transport_id}",
            },
            {
