...
class VisaApplicationDetailSerializer(serializers.ModelSerializer):
    ...
    approval_workflow = VisaApprovalStepSerializer(
        source="visaapprovalstep_set", many=True, read_only=True
    )
    documents = VisaDocumentSerializer(
        source="visadocument_set", many=True, read_only=True
    )
...
class VisaApplicationCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = VisaApplication
        fields = [
            # id must stay in the response so the frontend can submit the
            # application right after creating it (see visa-form.component.ts)
            "id",
    ...
    def validate(self, data):
        ...
        if data.get("passport_expiry_date"):
            ...
            if data["passport_expiry_date"] < today:
                raise serializers.ValidationError(
                    {"passport_expiry_date": "Passport has expired"}
                )
        ...
        if data.get("trip_start_date") and data.get("trip_end_date"):
            if data["trip_end_date"] < data["trip_start_date"]:
                raise serializers.ValidationError(
                    {"trip_end_date": "End date must be after start date"}
                )
