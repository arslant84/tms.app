...
REST_FRAMEWORK = {
    ...
    "DEFAULT_PAGINATION_CLASS": "utils.viewset_mixins.StandardResultsPagination",
...
WHITENOISE_ROOT = os.path.join(
    BASE_DIR.parent, "frontend", "dist", "tms-frontend", "browser"
)
...
DEFAULT_FROM_EMAIL = config(
    "DEFAULT_FROM_EMAIL", default="SynTra TMS <noreply@pctsb-travel.site>"
)
...
CONTENT_SECURITY_POLICY = {
    "DIRECTIVES": {
        ...
        "script-src": (
            "'self'",
            "'unsafe-inline'",
        ),  # 'unsafe-eval' moved to development.py (JIT-only)
        "script-src-elem": (
            "'self'",
            "'unsafe-inline'",
        ),  # <script> elements; no eval needed in production
        "script-src-attr": (
            "'unsafe-inline'",
        ),  # Angular build uses onload="this.media='all'" for deferred CSS loading
        "style-src": (
            "'self'",
            "'unsafe-inline'",
        ...
        "style-src-attr": (
            "'unsafe-inline'",
        ),  # Angular writes inline style="" attributes at runtime
        "img-src": (
            "'self'",
            "data:",
            "blob:",
        ),  # Removed wildcard https: scheme (ZAP: CSP Wildcard Directive)
        "font-src": ("'self'",),  # Fonts loaded locally via angular.json bundler
